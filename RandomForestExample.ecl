...
ClassifyingModel := learner.learnd(IndepData, DepData); // model to use when classifying
OUTPUT(ClassifyingModel,NAMED('ClassifyingModel'), ALL); // group_id represent number of tree
ReadableModel:= learner.model(ClassifyingModel);  // transforming model to a easier way to read it
...
ClassDist:= learner.ClassProbDistribD(IndepData, ClassifyingModel);
OUTPUT(ClassDist, NAMED('ClassDist'), ALL);
class:= learner.classifyD(IndepData, ClassifyingModel); // classifying
